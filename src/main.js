...
const createApp = () => {
  ...
  const { startAutoUpdate, stopAutoUpdate } = createUpdater(
    () => state,
    renderApp,
    renderPosts,
    generatePostId
  ...
  const renderFeeds = () => {
    ...
    container.innerHTML = state.feeds.length > 0
      ? `<div class="card border-0"><div class="card-body"><h2 class="card-title h4">${i18n.t('feeds.title')}</h2><ul class="list-group border-0 rounded-0">${
  ...
  const renderPosts = () => {
    ...
    container.innerHTML = state.posts.length > 0
      ? `<div class="card border-0"><div class="card-body"><h2 class="card-title h4">${i18n.t('posts.title')}</h2><ul class="list-group border-0 rounded-0">${
  ...
  return { init: () => i18n.changeLanguage('ru').then(renderApp) };
};
